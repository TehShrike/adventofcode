function makeActLikeSixteenBits(n) {
	return n & 65535
}

var gates = {
	LSHIFT: (n, bits) => makeActLikeSixteenBits(n << bits),
	RSHIFT: (n, bits) =>  makeActLikeSixteenBits(n >> bits),
	OR: (n, x) => makeActLikeSixteenBits(n | x),
	NOT: (n) => makeActLikeSixteenBits(~ n),
	AND: (n, x) => makeActLikeSixteenBits(n & x)
}
...
function performOperation(str) {
	var pieces = str.trim().split(/\s/)

	if (pieces.length === 1) {
		return getValue(str)
	} else if (pieces.length === 2) {
		return gates[pieces[0]](getValue(pieces[1]))
	} else {
		return gates[pieces[1]](getValue(pieces[0]), getValue(pieces[2]))
	}
}
...
function parseInstruction(str) {
	var operationAndOutput = str.split(' -> ')
	var operation = operationAndOutput[0]
	var outputWire = operationAndOutput[1].trim()

	wires[outputWire] = memoize(() => performOperation(operation))
}
...
// var instructions = ['123 -> x', '456 -> y', 'x AND y -> d', 'x OR y -> e', 'x LSHIFT 2 -> f', 'y RSHIFT 2 -> g', 'NOT x -> h', 'NOT y -> i']
