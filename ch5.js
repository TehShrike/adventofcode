...
function applyInstruction(grid, instruction) {
	applyToGrid(grid, instruction.fromX, instruction.fromY, instruction.toX, instruction.toY, instruction.instructionFn)
	return grid
}

function applyToGrid(grid, fromX, fromY, toX, toY, fn) {
	for (var i = fromX; i <= toX; ++i) {
		applyToRow(grid, i, fromY, toY, fn)
	}
}

function applyToRow(grid, x, fromY, toY, fn) {
	if (!grid[x]) {
		grid[x] = {}
	}

	for (var i = fromY; i <= toY; ++i) {
		grid[x][i] = fn(grid[x][i] || 0)
	}
}
...
var instructionRegex = /([\w ]+) (\d+),(\d+) through (\d+),(\d+)/

// console.log(instructionRegex.exec('toggle 461,550 through 564,900'))

var grid = instructions.map(function(instruction) {
	var match = instructionRegex.exec(instruction)

	return {
		instruction: match[1],
		fromX: parseInt(match[2], 10),
		fromY: parseInt(match[3], 10),
		toX: parseInt(match[4], 10),
		toY: parseInt(match[5], 10)
	}
}).map(function(instruction) {
	instruction.instructionFn = instructionFunctions[instruction.instruction]

	if (!instruction.instructionFn) {
		throw new Error('aaugh!')
	}

	return instruction
}).reduce(applyInstruction, {})

var totalOn = Object.keys(grid).map(function(x) {
	return grid[x]
}).reduce(function(total, row) {
	return total + Object.keys(row).map(function(y) {
		return row[y]
